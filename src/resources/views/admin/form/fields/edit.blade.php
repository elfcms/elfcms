...
@section('pagecontent')
    ...
    <div class="item-form">
        ...
        <form action="{{ route('admin.forms.fields.update', ['form' => $form, 'field' => $field]) }}" method="POST"
                    ...
                    <div class="input-wrapper">
                        <select name="group_id" id="group_id">
                            ...
                            @foreach ($groups as $item)
                                <option value="{{ $item->id }}" @if ($field->group && $item->id == $field->group->id) selected @endif>
                                    {{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="input-wrapper">
                        <select name="type_id" id="type_id">
                            @foreach ($types as $item)
                                <option value="{{ $item->id }}" @if ($item->id == $field->type->id) selected @endif>
                                    {{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                                ...
                                @foreach ($field->options as $item)
                                    <div class="options-table-string-line" data-exist-line="{{ $item->id }}">
                                        <div class="options-table-string">
                                            <input type="text" name="options_exist[{{ $item->id }}][value]"
                                                id="options_exist_value_{{ $item->id }}" data-option-value
                                                value="{{ $item->value }}">
                                        </div>
                                        <div class="options-table-string">
                                            <input type="text" name="options_exist[{{ $item->id }}][text]"
                                                id="options_exist_text_{{ $item->id }}" data-option-text
                                                value="{{ $item->text }}">
                                        </div>
                                        <div class="options-table-string">
                                            <input type="checkbox" name="options_exist[{{ $item->id }}][selected]"
                                                id="options_exist_selected_{{ $item->id }}" data-option-selected
                                                @if ($item->selected == 1) checked @endif>
                                        </div>
                                        <div class="options-table-string">
                                            <input type="checkbox" name="options_exist[{{ $item->id }}][disabled]"
                                                id="option_exist_disabled_{{ $item->id }}" data-option-disabled
                                                @if ($item->disabled == 1) checked @endif>
                                        </div>
                                        <div class="options-table-string">
                                            <input type="checkbox" name="options_exist[{{ $item->id }}][deleted]"
                                                id="options_exist_deleted_{{ $item->id }}" data-option-deleted>
                                        </div>
