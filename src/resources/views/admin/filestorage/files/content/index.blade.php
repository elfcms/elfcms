<form action="{{ route('admin.filestorage.files.groupSave',$filestorage) }}" method="POST" name="filestorage-file-list" id="filestorage-file-list">
    @method('POST')
    @csrf
    <div class="filestorage-files-content dragndrop-wrapper" draggable="false" data-uploadtext="{{__('elfcms::default.file_upload')}}">
        <a href="{{route('admin.filestorage.files.create',$filestorage)}}" class="filestorage-file-add filestorage-file-tile" id="filestoragefilecreate" title="{{__('elfcms::default.create_file')}}"></a>
...
<script>
const submitItems = document.querySelector('.filestorage-files-buttons .submit-button');
...
function filesUpload (files) {
    ...
    if (n > 0) {
        popup({
            title:'&nbsp;',
            content:'{{__("elfcms::default.following_files_will_be_downloaded")}}: <br><br>' + fileNames.join('<br>') + '<p>{{__("elfcms::default.are_you_sure")}}</p>',
            buttons:[
                {
                    title:'{{__("elfcms::default.cancel")}}',
                    class:'button',
...
function setItemData (file, data, empty=false, callback=null) {
    ...
    if (empty) {
        ...
        file.insertAdjacentHTML('beforeend',`<input type="hidden" name="file[${data.id}][position]" value="${data.position}" data-field="position">`);
    }
    else {
...
function createItem (data, empty = false) {
    ...
    if (!empty) {
        ...
        newItem.insertAdjacentHTML('beforeend',`<input type="hidden" name="file[${data.id}][position]" value="${data.position}" data-field="position">`);
    }
                            ...
                            (data) => {
                                if (data.result && data.result == 'success' && data.data) {
                                    ...
                                    if (isEdit) {
                                        ...
                                        newItem.insertAdjacentHTML('beforeend',`<input type="hidden" name="file[${data.data.id}][position]" value="${data.data.position}" data-field="position">`);
