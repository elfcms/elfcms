...
@section('innerpage-content')
    <div class="table-search-box">
        <a href="{{ route('admin.filestorage.groups.create') }}" class="default-btn success-button icon-text-button light-icon plus-button">{{__('elfcms::default.create_group')}}</a>
    ...
    <div class="widetable-wrapper">
        <table class="grid-table table-cols-8" style="--first-col:65px; --last-col:180px; --minw:800px">
            ...
            @foreach ($groups as $group)
                <tr data-id="{{ $group->id }}" @class(['inactive' => $group->active != 1])>
                    <td>{{ $group->id }}</td>
                    <td>
                        <a href="{{ route('admin.filestorage.groups.show',$group->id) }}">
                            {{ $group->title }}
                        </a>
                    </td>
                    <td>
                    ...
                    <td>{{ $group->active != 1 ? __('elfcms::default.inactive') : '' }}</td>
                    <td class="button-column non-text-buttons">
                        <a href="{{ route('admin.filestorage.groups.show',$group->id) }}" class="default-btn content-button" title="{{ __('elfcms::default.edit_storage_contents') }}"></a>
                        <a href="{{ route('admin.filestorage.groups.edit',$group->id) }}" class="default-btn edit-button" title="{{ __('elfcms::default.edit_storage_params') }}"></a>
                        <form action="{{ route('admin.filestorage.groups.destroy',$group->id) }}" method="POST" data-submit="check">
                            @csrf
                            @method('DELETE')
