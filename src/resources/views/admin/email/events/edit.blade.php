...
@section('pagecontent')
    <div class="table-search-box">
        <a href="{{ route('admin.email.events') }}" class="button round-button theme-button"
                    ...
                    <div class="input-wrapper">
                        <div>
                                ...
                                @forelse ($params as $param => $value)
                                    <div class="params-table-string-line" data-line="{{ $loop->index }}">
                                        <div class="params-table-string">
                                            <input type="text" name="params_new[{{ $loop->index }}][name]"
                                                id="param_new_name_{{ $loop->index }}" value="{{ $param }}"
                                                data-param-name>
                                        </div>
                                        <div class="params-table-string">
                                            <input type="text" name="params_new[{{ $loop->index }}][value]"
                                                id="param_new_value_{{ $loop->index }}" value="{{ $value }}"
                                                data-param-value>
                                        </div>
                                        <div class="params-table-string">
                                            <button type="button" class="button"
                                                onclick="eventParamDelete({{ $loop->index }})">&#215;</button>
                                        </div>
                                    </div>
                                @empty
                                    <div class="params-table-string-line" data-line="0">
                                        <div class="params-table-string">
                                            <input type="text" name="params_new[0][name]" id="param_new_name_0"
                                                data-param-name>
                                        </div>
                                        <div class="params-table-string">
                                            <input type="text" name="params_new[0][value]" id="param_new_value_0"
                                                data-param-value>
                                        </div>
                                        <div class="params-table-string">
                                            <button type="button" class="button"
                                                onclick="eventParamDelete(0)">&#215;</button>
                                        </div>
                                    </div>
                                @endforelse
                            ...
                            <button type="button" class="button simple-button"
                                id="addparamline">{{ __('elfcms::default.add_parameter') }}</button>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="input-wrapper">
                        <select name="view" id="view">
                            ...
                            @foreach ($views as $view)
                                <option value="{{ $view }}" @if ($view == $event->view) selected @endif>
                                    {{ $view }}</option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        <div class="input-wrapper">
                            ...
                            <select name="{{ $fieldName }}" id="{{ $fieldName }}">
                                ...
                                @foreach ($addresses as $address)
                                    <option value="{{ $address->id }}" @if (isset($fields[$fieldName]) && $fields[$fieldName]->email == $address->email) selected @endif>
                                        {{ $address->name }} &lt;{{ $address->email }}&gt;</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
