...
const fieldContainers = Array.from(
    document.querySelectorAll(".form-group-fields, .form-groupless-fields")
);
...
function getDragAfterItem(container, y) {
    const draggableElements = Array.from(
        container.querySelectorAll(".form-field-box:not(.dragging)")
    );
...
function itemPositionSuccess() {
    ...
    if (!fieldContainers || !formGroups || !formGroups.dataset.form)
        return false;
        ...
        if (boxFields) {
            boxFields.forEach((field) => {
                if (field.dataset.id) {
                    ...
                    if (
                        preFields[field.dataset.id] !== undefined &&
                        fieldsData[field.dataset.id] !== undefined
                    ) {
                        if (
                            preFields[field.dataset.id].position !=
                                fieldsData[field.dataset.id].position ||
                            preFields[field.dataset.id].group !=
                                fieldsData[field.dataset.id].group
                        ) {
                            changeCount++;
                            isNew = true;
                        }
                    ...
                    const positionBox = field.querySelector(
                        ".form-field-position"
                    );
                    if (positionBox) {
